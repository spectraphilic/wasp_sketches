            ...
            for (uint8_t j = 0; j < n; j++) {
                if (j > 0) {
                    ...
                    if (value > -128 && value < 128) {
                        if (addSensorValue((int8_t)value) == 0) {
                            err = -1;
                            goto exit;
                        }
                        continue;
                    ...
                    if (addSensorValue((int8_t)-128) == 0) {
                        err = -1;
                        goto exit;
                    }
                }

                if (addSensorValue(values[j]) == 0) {
                    err = -1;
                    goto exit;
                }
            }
...
int WaspUIO::readFrame(uint8_t &n)
{
    ...
    while (true) {
        ...
        // Read the frame
        if (getDataFilename(dataFilename, item[0], item[1], item[2], item[3])) {
            return -1;
        }
