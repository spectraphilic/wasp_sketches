...
{	
	for (int i=0; i<4; i++)
	{
		...
		I2CReadWord(MLX90614_REGISTER_ID0 + i, &temp);
...
uint8_t IRTherm::sleep(void)
{
	...
	Wire.beginTransmission(_deviceAddress);
     ...
     Wire.send(MLX90614_REGISTER_SLEEP);
	//Wire.write(crc);
     Wire.send(crc);
	//Wire.endTransmission(true);
	Wire.secureEnd();

	// Set the I2C_SCL pin LOW, and I2C_SDA pin HIGH (should be pulled up)
...
uint8_t IRTherm::wake(void)
{
	...
	pinMode(I2C_SCL, INPUT); // I2C_SCL high
	pinMode(I2C_SDA, OUTPUT);
	digitalWrite(I2C_SDA, LOW); // I2C_SDA low
	delay(50); // delay at least 33ms
	pinMode(I2C_SDA, INPUT); // I2C_SDA high
	delay(250);
	// PWM to SMBus mode:
	pinMode(I2C_SCL, OUTPUT);
	digitalWrite(I2C_SCL, LOW); // I2C_SCL low
...
void IRTherm::I2CWriteWord(byte reg, int16_t data)
{
	uint8_t crc;
