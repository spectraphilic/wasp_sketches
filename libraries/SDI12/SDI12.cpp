...
/* Read the response from the sensor into the buffer. Return 0 on success, 1 on
 * error. */
...
uint8_t SDI12::command2address(uint8_t address, const char* cmd)
{
  ...
  if (address > 9)
  {
    error(F("command2address(%d, %s): unexpected address, must be 0-9"), address, cmd);
    return 1;
  }
  ...
  if (n < 0 || n >= max)
  {
    error(F("command2address(%d, %s): snprintf failed %d"), address, cmd, n);
    return 1;
  }
  ...
  if (readline() != 0)
  {
    error(F("command2address(%s): fail, read '%s'"), aux, buffer);
    return 1;
  }
...
char SDI12::read_address()
{
  sendCommand("?!");
  if (readline() != 0)
  {
    error(F("read_address(): fail, read %s"), buffer);
    return 0;
  }

  debug(F("read_address: success, read '%s'"), buffer);
  return buffer[0];
}

uint8_t SDI12::set_address(char current_address, char new_address)
{
  char aux[5];
  ...
  if (readline() != 0)
  {
    error(F("reading response in set_address, read %s"), buffer);
    return 1;
  }

  if (buffer[0] != new_address)
  {
    error(F("set_address failed, expected %c read %s"), new_address, buffer);
    return 1;
  }
